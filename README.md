...
- 無音の窓は捨てる：自前でもマイク音量を測り、窓全体が無音ならモデルの幻覚
  （無音に「ごめん」等を出す実測あり）として追記しない。発話があれば必ず通す。
  監視に失敗したら無条件で通す（欠落より残存を選ぶ）。閾値は
  `WindowSpeechGate.threshold`（既定 RMS 0.01、実機のノイズフロアに合わせる）。
