...
以下の VAD に関する記述は旧方式の記録。現在の起動引数は
`--step 5000 --length 5000 --keep 0`。音声窓を重ねず、`DuplicateGuard` は
本番経路では使わない（旧方式のテスト用に残している）。PoC の Python 経路の
ローリング窓・安定化ロジックを移植したものではない。

- whisper.cpp v1.9.2 の非 VAD 経路は ANSI 行消去＋本文＋改行を出す。
  `step == length` なら毎推論で改行するため、これを確定境界としてパースする。
  stdout は行が揃うまでバイトで保持し、日本語の UTF-8 分断に耐える。
- 通常停止は SIGINT → SDL の終了経路 → 残り音声を推論 → stdout を EOF まで回収。
  完了通知より前に本文通知を main queue に配送し、停止時コピーに末尾も含める。
  `.finishing` 中は開始・停止を重ねない。15秒で強制終了し、失敗を表示する。
- 起動キャンセル・アプリ終了は `shutdown()`（即時停止、末尾の回収保証なし）。
- `bash Tools/test.sh`：旧テスト＋非重複窓の引数／パーサ＋模擬プロセスでの末尾回収。
  実エンジンの短い窓は `Tools/engine-smoke.swift` で確認できる。
...
1. ✅ **別マシンでの不具合2件**（2026-09-08、PR #1 #2 で修正、v0.1.3）:
